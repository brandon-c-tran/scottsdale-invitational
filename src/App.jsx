...
export default function App() {
    ...
    if (version > prevVersion.current && prevVersion.current > 0) {
      ...
      if (me && settleToastV.current !== version) {
        if (lastAction === "saveResult") {
          ...
          if (award > 0) notify(`You took ${["1st","2nd","3rd"][idx]}, +${award}`, null, "gold", me);
        }
  ...
  const simPokerNight = async ({ through = "result" } = {}) => {
    ...
    if (todo.length) {
      ...
      let left = Math.max(0, Math.floor((pokerChips(poker.total) - counted) / CHIP_MIN));
      ...
      <Shell>
        <Onboarding step={onboardStep} me={me} state={state} onTv={() => setTv(true)} onChip={pickChip}
          ...
          next={() => setOnboardStep(s => s + 1)}
            ...
            saveMine("si-onboard-epoch", String(state.onboardEpoch || 0)); setTab("board"); setTour(0); }} />
        <Confetti burst={burst} />
      </Shell>
    );
  }
    ...
    <Shell>
      ...
      {modal?.type === "qa" && <QASheet rank={simRank(state)} presets={QA_PRESETS} busy={!!sim}
        ...
        onCountRest={async () => {
          ...
          let left = Math.max(0, Math.floor((pokerChips(p.total) - counted) / CHIP_MIN));
...
function LockerRoom({ state, me, gm, onProfile, onStart, onChallenge }) {
...
const NEED_CHIP = [
  ...
  { bg:"var(--sun)",   fg:"var(--ink0)", text:"2nd or better" },
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            {myRow && d ? (
              <div style={{ fontFamily:SANS, fontSize:12.5, color:BONE, marginTop:2 }}>
                Your buy-in: <b>{fmt(pokerChips(myRow.pts))}</b>
                {d.length ? `, take ${d.map(x => `${x.n} x ${x.v}`).join(" + ")}` : ""}
              </div>
            ) : (
              <div style={{ fontFamily:SANS, fontSize:11.5, color:"var(--night-text)" }}>{fmt(pokerChips(pk.total))} chips in play</div>
            )}
          </div>
          ...
          {pk.counts?.[me] !== undefined && !counting ? (
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <span style={{ fontFamily:SANS, fontSize:12.5, color:"var(--night-text)", flex:1 }}>
                Your stack: <b style={{ color:BONE }}>{fmt(pokerChips(myRow?.pts ?? 0))}</b> at the buy-in</span>
              <button onClick={() => setCounting(true)} style={{ background:"none", border:"none",
                color:"var(--night-text)", fontFamily:SANS, fontWeight:700, fontSize:11.5, cursor:"pointer",
      ...
      {counted.length > 0 && (
        <div style={{ borderTop:"1px solid rgba(251,243,228,0.12)", padding:"8px 14px",
          ...
          <span style={{ fontFamily:SANS, fontSize:12, color:"var(--night-text)", flex:1 }}>
            {counted.length} of {alive} counted{allIn ? `, ${fmt(countSum)} of ${fmt(pokerChips(pk.total))}` : ""}
            {allIn && countSum !== pokerChips(pk.total) && (
              <span style={{ color:"var(--clay)" }}> ({countSum > pokerChips(pk.total)
                ? `${fmt(countSum - pokerChips(pk.total))} over` : `${fmt(pokerChips(pk.total) - countSum)} short`})</span>
            )}
          </span>
          {gm && (
...
function PokerBuyinSheet({ state, standings, gm, onClose, onStart }) {
  ...
  return (
    <Sheet title="Buy-in" onClose={onClose}>
      <p style={pStyle}>Points add a zero and become chips. Everyone takes their own stack from the tray.</p>
      {standings.map(r => {
        ...
        return (
          <div key={r.player} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0",
            ...
            <span style={{ fontFamily:DISPLAY, fontWeight:700, fontSize:19, color:"var(--ink)", minWidth:52,
              textAlign:"right" }}>{fmt(pokerChips(r.pts))}</span>
          </div>
        );
      })}
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0 14px" }}>
        <span style={{ ...label, flex:1 }}>Chips in play</span>
        <span style={{ fontFamily:DISPLAY, fontWeight:700, fontSize:22, color:"var(--sun)" }}>{fmt(pokerChips(pk.total))}</span>
      </div>
    ...
    <Sheet title="The table" onClose={onClose}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 0 4px" }}>
        ...
        <span style={{ fontFamily:DISPLAY, fontWeight:700, fontSize:22,
          color: allIn && sum === pokerChips(pk.total) ? "var(--green)" : "var(--ink)" }}>{fmt(sum)}</span>
        <span style={{ fontFamily:SANS, fontSize:12.5, color:"var(--muted)" }}>of {fmt(pokerChips(pk.total))}</span>
      </div>
      {allIn && sum !== pokerChips(pk.total) && (
        <div style={{ fontFamily:SANS, fontSize:12.5, color:"var(--clay)", marginBottom:10 }}>
          {sum > pokerChips(pk.total) ? `${fmt(sum - pokerChips(pk.total))} over` : `${fmt(pokerChips(pk.total) - sum)} short`}. Chips get miscounted, you can still post.
        </div>
      )}
          ...
          return (
            <div key={r.player}
              ...
              <div key={r.pts} style={{ fontFamily:DISPLAY, fontWeight:700, fontSize: first ? 30 : 24,
                minWidth:36, textAlign:"right", color: first ? "var(--ink0)" : "var(--ink)", animation:"si-pop .5s ease-out" }}>{r.pts}</div>
            </div>
          );
        })}
      </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14, background:CARD_BG,
          ...
          <div style={{ textAlign:"center" }}>
            {(() => {
              ...
              const slots = myCap / PT;
              const size = slots <= 4 ? 18 : slots <= 7 ? 14 : 11;
              return (
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {row && (
            <div style={{ fontFamily:SANS, fontSize:12.5, color:"var(--muted)", marginTop:4 }}>
              {["1st","2nd","3rd"][row.rank - 1] || `${row.rank}th`} on the board, {row.pts} pts, {row.wins} win{row.wins === 1 ? "" : "s"}
...
const chipMarks = (skin, cx = 16, edge = 12.4) => {
  ...
  const lines = (n, off, r1, r2, w) => Array.from({ length: n }, (_, i) => {
    ...
    return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(251,243,228,0.75)"
      strokeWidth={w} strokeLinecap="round" />;
  });
  if (skin === "plain") return null;
  if (skin === "dash") return <circle cx={cx} cy={cx} r={edge - 1.4} fill="none"
    stroke="rgba(251,243,228,0.75)" strokeWidth="2.6" strokeDasharray="7.2 10.1" />;
  if (skin === "ring") return <circle cx={cx} cy={cx} r={edge - 3.4} fill="none"
    stroke="rgba(251,243,228,0.75)" strokeWidth="1.8" />;
  ...
  if (skin === "dots") return Array.from({ length: 8 }, (_, i) => {
    ...
    return <circle key={i} cx={x} cy={y} r="1.7" fill="rgba(251,243,228,0.75)" />;
  });
...
function TVBettingBoard({ state, events, ev }) {
  ...
  const Hero = GAME_HEROES[ev.game];
  const chipsIn = open.reduce((n, x) => n + x.w.stake, 0);
    ...
    <div style={{ width:320, flexShrink:0, background:CARD_BG, border:"1px solid var(--line)",
      ...
      {standings.map((r, i) => (
        <div key={r.player} style={{ display:"flex", alignItems:"center", gap:10,
          ...
          <span style={{ fontFamily:SANS, fontWeight:700, fontSize:14, flex:1,
        ...
        <div style={{ display:"flex", alignItems:"center", gap:20, marginBottom:16 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily:DISPLAY, fontWeight:700, fontSize:"clamp(28px,2.8vw,44px)",
              textTransform:"uppercase", color:BONE }}>Your points add a zero.</div>
          </div>
          <div style={{ marginLeft:"auto", textAlign:"right" }}>
            <div style={{ fontFamily:DISPLAY, fontWeight:700, fontSize:"clamp(28px,3vw,48px)", color:"var(--sun)" }}>{fmt(pokerChips(pk.total))}</div>
          ...
          {standings.map(r => {
            ...
            return (
              <div key={r.player} style={{ display:"flex", alignItems:"center", gap:12, background:CARD_BG,
                ...
                <div style={{ fontFamily:DISPLAY, fontWeight:700, fontSize:"clamp(22px,2.2vw,34px)", color:"var(--sun)" }}>{fmt(pokerChips(r.pts))}</div>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div style={{ position:"fixed", inset:0, display:"flex", flexDirection:"column", zIndex:60,
      ...
      <button onClick={onExit} style={{ position:"absolute", top:"calc(16px + env(safe-area-inset-top))", right:16, zIndex:70,
        background:"var(--paper)", border:"1.5px solid rgba(251,243,228,0.28)", color:"var(--ink)",
